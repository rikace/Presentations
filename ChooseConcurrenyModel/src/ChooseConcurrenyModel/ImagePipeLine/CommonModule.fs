...
module StartPhotoViewer = 
    let private path = System.IO.Path.Combine(System.IO.Path.GetDirectoryName(__SOURCE_DIRECTORY__ ),@"Helpers\PhtotoViewer\PhotoViewer\bin\Debug\PhotoViewer.exe")
    let start() = System.Diagnostics.Process.Start(path)
