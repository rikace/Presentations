...
module ExampleQueryPredicate =
    ...
    let db = new SimpleDbContext("Data Source=DESKTOP-0G0BUJ0;Initial Catalog=SimpleDB;Integrated Security=SSPI;")
    ...
    let pred =
        ...
        |> List.map(fun id ->
                ...
                | None -> pred
                | Some yearIds -> And(pred,  Condition(ContainsElements(InMemoryNullInts(nlst),GetColumn("Age")))))
    ...
    let shouldEqual (value : QueryValue) (colName : string) = Condition(Equals(GetColumn(colName), Constant(value)))



    let predEQ = "Age" |> shouldEqual (NullNumber(Nullable(10)))
