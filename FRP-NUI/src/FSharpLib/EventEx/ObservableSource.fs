...
module ObservableSource = 
    ...
    let left, right = form.MouseDown
                        |> Observable.merge form.MouseMove
                        |> Observable.filter ( fun args -> args.Button = MouseButtons.Left || args.Button = MouseButtons.Right)
                        |> Observable.map (fun args -> args.X, args.Y, args.Button)
                        |> Observable.filter( fun(x,y,b) -> x < 100 && y > 100)
                        |> Observable.partition (fun (_, _, b) -> b = MouseButtons.Left)
    let leftSubscription =
          left
            |> Observable.subscribe (fun (x,y,_) -> printfn "Left (%d,%d)" x y)
