...
one. (Immutability and transformation)

The behavior of Mario is composed from 4 functions that represent different aspects 
of the game.
...
(**
The `step` function is the result of the composition of the function above.
The function takes a `dir` parameter representing the keyboard status
and a current `Mario` state. It simply runs 4 components in a pipeline:
*)

// int * int -> Mario -> Mario 
let step dir mario = mario |> jump dir |> walk dir  |> physics 
