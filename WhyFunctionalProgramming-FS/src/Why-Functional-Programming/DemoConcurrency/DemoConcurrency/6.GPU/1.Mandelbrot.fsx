...
module MandelbrotKernels = 
    ...
    let MandelbrotExecution = fun ((ctx:FsGPU.Cuda.ExecutionContext), (width : int, height : int, crunch : int, xOff : float32, yOff: float32, scale : float32)) ->
        let (exec:GASS.CUDA.Engine.CUDAExecution) = ctx.Execution                                               
...
module Mandelbrot =
    ...
    let calcColors (bw:int) (bh:int) maxit map =
        ...
        let num = bw * bh
        MandelbrotKernels.CalcMandelbrot(bw, bh, maxit, xOff, yOff, scale)
    ...
    let TestMandelbrotGPU() = 
        ...
        MandelbrotKernels.CalcMandelbrot(width, height, maxit, xOff, yOff, scale)
