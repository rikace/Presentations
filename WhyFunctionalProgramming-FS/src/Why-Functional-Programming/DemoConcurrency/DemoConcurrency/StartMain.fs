...
using((new OperationTimer(Some("Test Build Rec"))))(fun _ -> 
                                    let rnd = System.Random()
                                    let rec build acc n =
                                        match n with
                                        | 0 -> acc
                                        | n -> build ((rnd.Next(100))::acc) (n - 1)
                                    let res = build [] 10000000
                                    printf "Count %d" (List.length res)
                                    ())
                                    
using((new OperationTimer(Some("Test Build List"))))(fun _ -> 
                                    let c = 10000000
                                    let rnd = System.Random()
                                    let lst = new System.Collections.Generic.List<int>()
                                    for i = 0 to c do
                                        lst.Add((rnd.Next(100)))
                                    printf "Count %d" lst.Count
                                    let res = lst
                                    ())
