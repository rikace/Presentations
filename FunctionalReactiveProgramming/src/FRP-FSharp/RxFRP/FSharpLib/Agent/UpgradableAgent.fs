...
| Upgrade of ('x*'state -> 'x*'state) // function what to do
    ...
    let gen_server = MailboxProcessor.Start(fun msg ->
        let rec loop (state, f) =
            async { 
                ...
                match receive with
                | Upgrade(f1,f2) -> let state1 = f2(state)
                                    return! loop(state1, f1)
                | From(from, x) ->  let (reply, state1) = f(x,state)
                                    from.Reply(reply)
...
module TestUpgradableAgent =
    ...
    let test3 =
        ...
        //let myfunc4 ((x:string), (state:obj)) = (x+unbox(state), box(x+unbox(state))) 
