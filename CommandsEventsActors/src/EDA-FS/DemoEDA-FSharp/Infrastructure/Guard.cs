...
namespace DemoEDA.Infrastructure
{
    internal static class Guard
    {
        ...
        internal static void NotNull<T>(T value, string errorMessage = null)
        {
            if (ReferenceEquals(value, null))
            {
                throw new ArgumentNullException(String.IsNullOrEmpty(errorMessage) ? value.GetType().Name : errorMessage);
            }
        }
            ...
            where T : IComparable
        {
            ...
            if (value.CompareTo(compareTo) <= 0)
            {
                throw new ArgumentOutOfRangeException(
                    "{0} has value {1} which is less than or equal to {2}".FormatWith(GetParameterName(reference), value,
                        compareTo));
            }
        }
            ...
            where T : IComparable
        {
            ...
            {
                throw new ArgumentOutOfRangeException(
                    "{0} has value {1} which is less than {2}".FormatWith(GetParameterName(reference), value, compareTo));
            }
        }
            ...
            where T : IComparable
        {
            ...
            if (value.CompareTo(default(T)) == 0)
            {
                throw new ArgumentException(
                    "{0} has value {1} which cannot be equal to it's default value {2}".FormatWith(
        ...
        internal static void NotEmpty<T>(Expression<Func<IEnumerable<T>>> reference, IEnumerable<T> value)
        {
            ...
            if (!value.Any())
            {
                throw new ArgumentException("{0} cannot be empty".FormatWith(GetParameterName(reference), value,
                    default(T)));
            }
        }
