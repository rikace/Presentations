[<AutoOpenAttribute>]
module StockTicker.TradingSupervisorAgent
...
open TradingAgent
open StockTicker.Server.Common

// TradingSuperviser agent based to handle active trading children agent
...
type TradingCoordinator() =   // #A
    ...
    interface IDisposable with   // #O
        member x.Dispose() =  (subject:>IDisposable).Dispose()