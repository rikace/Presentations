...
namespace StockTicker.Server.Cs
{
    public class Startup
    {
        public Startup()
        {
            agent = new ActionBlock<CommandWrapper>(async cmd => {
                await Commands.CommandHandler.Handle(cmd);
            });
        }
        ...
        public void Configuration(IAppBuilder builder)
        {
            ...
            config.Services.Replace(
                ...
                new ControlActivatorPublisher(
                    Observer.Create<CommandWrapper>(
                        command => agent.Post<CommandWrapper>(command))));
            ...
            TradingSupervisorAgent.TradingCoordinator.Instance()
