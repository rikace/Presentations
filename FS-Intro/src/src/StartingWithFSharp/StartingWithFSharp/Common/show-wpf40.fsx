...
let showA x = 
    ...
    | :? ScrollViewer -> let viewer = (win.Content :?> ScrollViewer)
                         let textBox = (viewer.Content :?> TextBlock)
                         textBox.Text <- sprintf "%s\n%A" textBox.Text x
                         viewer.ScrollToBottom()
    | _ ->  let viewer = ScrollViewer()
            let x = 
                match box x with 
                | :? string as s -> box s
                | :? System.Collections.IEnumerable as e -> e |> Seq.cast |> Seq.truncate 100 |> Seq.toList |> box
...
let randomColor() =
  ...
  let b' x = byte(x)
  Color.FromRgb(b'(r), b'(g), b'(b))

let showAC x = 
    match win.Content with
    | :? ScrollViewer -> let viewer = (win.Content :?> ScrollViewer)
                         let textBox = (viewer.Content :?> TextBlock)
                         let run = new Run(sprintf "%s\n%A" textBox.Text x)
                         run.Foreground <-  new SolidColorBrush(randomColor())
